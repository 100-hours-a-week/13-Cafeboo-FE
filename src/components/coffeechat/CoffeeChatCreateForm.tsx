...
const LocationSelector = React.lazy(
  () => import('@/components/coffeechat/LocationSelector')
);
...
const CoffeeChatSchema = z.object({
  title: z
    .string()
    .min(1, '커피챗 이름을 입력하세요')
    .max(20, '최대 20자까지 입력 가능'),
  content: z
    .string()
    .min(1, '상세 내용을 입력하세요')
    .max(200, '최대 200자까지 입력 가능'),
  chatNickname: z
    .string()
    .min(1, '채팅방 닉네임을 입력하세요')
    .max(10, '최대 10자까지 입력 가능'),
  memberCount: z.number().min(2).max(30),
  tags: z
    .array(z.string().max(15))
    .max(5, '태그는 최대 5개까지 등록 가능합니다')
    .optional(),
  time: z.string().min(1, '만나는 시간을 선택하세요.').refine(isFutureTime, {
    message: '현재 시간 이후로만 설정할 수 있습니다.',
  }),
  location: z.object({
...
export default function CoffeeChatForm({
  ...
  const incrementmemberCount = () =>
    setMemberCount((prev) => Math.min(30, prev + 1));
          ...
          <AccordionItem value="time" className="border-b border-gray-200">
            ...
            >
              <div className="flex items-center justify-between w-full">
                ...
                {time && <span className="text-gray-500 mr-1">{time}</span>}
              </div>
            </AccordionTrigger>
                ...
                {tags.length > 0 && (
                  <div className="flex flex-wrap gap-1 mr-1">
                    {tags.slice(0, 2).map((tag, index) => (
                      <span
                        key={index}
                        className="bg-gray-100 text-gray-600 px-2 py-1 text-xs rounded-xs max-w-[60px] truncate inline-block"
                      >
                        # {tag}
                      </span>
                    ))}
                ...
                {location?.placeName && location?.detailAddress && (
                  <div className="flex items-center">
                    ...
                    <Popover>
                      ...
                      <PopoverContent
                        className="w-70 z-[100] absolute right-[-5px]"
                        sideOffset={10}
                      >
                        <div className="flex items-center space-x-3">
                          ...
                          <div className="flex-1">
                            ...
                            <p className="font-medium text-gray-800">
                              {extractAreaUnit(location.detailAddress)}
                            </p>
                          </div>
                        </div>
